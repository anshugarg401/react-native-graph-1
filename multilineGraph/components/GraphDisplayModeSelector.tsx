...
import {
  ...
  SCREEN_WIDTH,
  graphs
} from '../constants/GraphDisplayConstant'
...
export function GraphDisplayModeSelector({
  ...
  return (
    <View {...props} onLayout={onLayout} style={[styles.container, style]}>
      ...
      />
      {graphs.map((displayMode) => (
        <View key={displayMode.value} style={styles.buttonContainer}>
          <PressableScale
            style={styles.button}
            onPress={() => setGraphDisplayMode(displayMode.value)}
          >
