...
// import type { AnimatedLineGraphProps } from 'react-native-graph/src/LineGraphProps'
import { SelectionDot as DefaultSelectionDot} from 'react-native-graph/src/SelectionDot'
// import DefaultSelectionDot from './Cursor'
import {
  createGraphPath,
...
import { getSixDigitHex } from 'react-native-graph/src/utils/getSixDigitHex'
import { usePanGesture } from "react-native-graph/src/hooks/usePanGesture"
import { getYForX } from 'react-native-graph/src/GetYForX'
import { hexToRgba } from 'react-native-graph/src/utils/hexToRgba'
...
const INDICATOR_PULSE_BLUR_RADIUS_BIG =
  ...
  export interface AnimatedLineGraphProps {
    ...
    onPointSelected?: (point: { date: number; value: number }) => void;
  ...
  const setFingerPoint = useCallback(
    (fingerX: number) => {
      ...
      if (pointSelectedIndex.current !== pointIndex) {
        ...
        if (dataPoint != null) {
          onPointSelected?.({ date: dataPoint.date.getTime(), value: dataPoint.value })
        }
      }
    },
    [
